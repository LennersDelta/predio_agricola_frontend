...
function EditarBoletaHonorarioPageInner() {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      try {
        ...
        const { data } = await api.get(`/api/boletahonorario/${uuid}`);
        ...
        console.error(error);
        toast.error('No se pudo cargar la boleta de honorario');
      } finally {
  ...
  const validate = () => {
    const newErrors: Record<string, string> = {};
        ...
        if (!form.boleta)
        newErrors.boleta = 'Debe ingresar la boleta.';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formData = new FormData();
        ...
        formData.append('boleta', form.boleta);
      ...
      await api.post(`/api/boletahonorario/${uuid}`, formData, {
        headers: {
      ...
      <div style={{ fontFamily: '"Barlow",sans-serif' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24, padding: '0 4px' }}>
          <div>
            ...
            <h2 style={{ fontFamily: '"Barlow Condensed",sans-serif', fontSize: '2.2rem', fontWeight: 800, color: '#1a2e22', textTransform: 'uppercase', letterSpacing: '.06em', lineHeight: 1, marginBottom: 6 }}>Editar boleta honorario</h2>
          </div>
          <Link href={`/predio/boletashonorarios/`} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 18px', borderRadius: 8, fontFamily: '"Barlow Condensed",sans-serif', fontSize: '.8rem', fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: '#1a2e22', textDecoration: 'none', background: 'linear-gradient(135deg,#8a6a18,#d4a832)', boxShadow: '0 4px 14px rgba(201,168,76,.3)' }} onMouseEnter={e => (e.currentTarget.style.filter = 'brightness(1.1)')} onMouseLeave={e => (e.currentTarget.style.filter = '')}>
...
export default function EditarBoletaHonorarioPage() {
  return <Suspense><EditarBoletaHonorarioPageInner /></Suspense>;
}