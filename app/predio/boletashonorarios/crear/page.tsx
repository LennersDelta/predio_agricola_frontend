// app/predio/boletahonorario/crear/page.tsx
'use client';
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  if (!form.mes)
    errsFront.mes = 'Debe seleccionar el mes.';

  if (!form.item)
    errsFront.item = 'Debe ingresar el ítem.';

  if (!form.total)
    errsFront.total = 'Debe ingresar el total.';

  if (!form.fecha)
    errsFront.fecha = 'Debe ingresar la fecha.';

  if (!form.boleta)
    errsFront.boleta = 'Debe ingresar la boleta.';

  if (!form.doe_informa_ab5)
    errsFront.doe_informa_ab5 = 'Debe indicar si informó a AB5.';

  if (!form.observaciones)
    errsFront.observaciones = 'Debe ingresar las observaciones.';
  ...
  try {
    ...
    await api.post('/api/boletahonorario/insert', fd, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });

    toast.success('Boleta honorario guardada correctamente.');
    setTimeout(() => {
      router.push('/predio/boletashonorarios');
    }, 1000);

    } catch (err: any) {

    // Registro duplicado (Predio + Mes)
    if (err.response?.status === 422) {

        setModalMessage(
        err.response?.data?.message ??
        'Ya existe una boleta honorario para el predio y mes seleccionados.'
        );

        const nombrePredio = predios.find(
        (p) => p.id === Number(form.predio_id)
        )?.nombre ?? '';

        setModalData({
        predio: nombrePredio,
        mes: formatoMes(form.mes)
        });
    ...
    toast.error(
        err.response?.data?.message ?? 'Error al guardar la boleta honorario.',
        {
        duration: 5000,
        }
    );
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between',
          ...
          <div>
            ...
            <h2 style={{ fontFamily: '"Barlow Condensed",sans-serif', fontSize: '2.2rem',
              ...
              Nueva Boleta de Honorario
            </h2>
            <p style={{ fontSize: '.72rem', color: '#3d5c47', fontFamily: 'monospace' }}>
               Complete la información de la boleta de honorario.
            </p>
          </div>
          <Link href="/predio/boletashonorarios"
        ...
        <form onSubmit={handleSubmit}>
            ...
            >
                ...
                >
                    <FInput
                        value={form.boleta}
                        onChange={e => set('boleta', e.target.value)}
                    />
                </Field>
            </div>
            </Section>
            ...
            paddingRight: 20 
                ...
                >
                    {loading ? 'Guardando...' : 'Guardar boleta honorario'}
                </button>
            </div>
            </div>
        </form>
