    ...
    <div onClick={e => { if (e.target === e.currentTarget) onCancel(); }}
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 14, padding: '32px 28px', maxWidth: 340, width: '90%', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,.1)' }}>
        ...
        <p style={{ fontSize: '.78rem', color: '#6b8f75', lineHeight: 1.6, marginBottom: 24 }}>
          Esta acción no se puede deshacer.<br />Boleta honorario será eliminado permanentemente.
        </p>
...
function BoletaHonorarioPageInner() {
  const searchParams = useSearchParams();
  const [data,         setData]         = useState<BoletaHonorario[]>([]);
...
// Tabla
  ...
  // ── Cargar datos ──────────────────────────────────────────────────────────
    const cargaBoletaHonorario = useCallback(() => {
    setLoading(true);

    api.get('/api/listaBoletaHonorario')
        .then(({ data }) => {
        let datos = [];
        ...
        .catch((err) => {
        console.error(err);
        toast.error('Error al cargar boleta honorario');
        setData([]);
        })
        .finally(() => {
        setLoading(false);
    ...
    const handleDelete = async () => {
      ...
      try {
        await api.delete(`/api/deleteBoletaHonorario/${deleteId}`);
        setData(prev => prev.filter(b => b.orden !== deleteId));
        toast.success('Boleta honorario eliminado correctamente', { id: toastId, duration: 3000 });
      } catch (err: any) {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24, padding: '0 4px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '3px 10px 3px 8px', borderRadius: 999, background: 'rgba(58,153,86,.1)', border: '1px solid rgba(58,153,86,.25)' }}>
            ...
            <span style={{ fontFamily: 'monospace', fontSize: '.58rem', fontWeight: 500, color: '#2e7d46', letterSpacing: '.12em', textTransform: 'uppercase' }}>Boleta Honorario</span>
          </div>
          ...
          <p style={{ fontSize: '.72rem', color: '#3d5c47', fontFamily: 'monospace' }}>Gestión Boletas Honorarios</p>
        </div>
        <Link href="/predio/boletashonorarios/crear"
                      ...
                      >
                        ...
                        <td style={{ padding: '10px 14px', verticalAlign: 'middle' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                            <Link href={`/predio/boletashonorarios/${b.uuid}/ver`}
                            ...
                            <Link href={`/predio/boletashonorarios/${b.uuid}/edit`}
