...
function EditarCompra3utmPageInner() {
    ...
    const { estados: EstadoCompra, loading: loadingEstadoCompra, error: errorEstadoCompra } = useEstados('estadoFactura');
...
const set = (k: string, v: string) => {
  ...
  const validate = () => {
    ...
    if (!form.proveedor)
      newErrors.proveedor = 'Debe ingresar el proveedor.';

    if (!form.factura)
      newErrors.factura = 'Debe ingresar la factura.';
    ...
    if (!form.materia)
      newErrors.materia = 'Debe ingresar la materia.';

    if (!form.estado_id)
      newErrors.estado_id = 'Debe seleccionar el estado.';

    if (!form.doe_envio_ab5)
      newErrors.doe_envio_ab5 = 'Debe ingresar DOE envío a pago.';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('estado_id', String(form.estado_id));
      formData.append('doe_envio_ab5', form.doe_envio_ab5);

      if (form.observaciones) {
        formData.append('observaciones', form.observaciones);
      }

      await api.post(`/api/compra3utm/${uuid}`, formData, {
        headers: {
        ...
        <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <Field label="Predio" error={errors.predio_id}>
                ...
                <Field label="Factura" error={errors.factura}>
                <FInput
                    value={form.factura}
                ...
                <Field label="Materia" error={errors.materia}>
                <FInput
                    value={form.materia}
            ...
            >
                ...
                <Field
                label="DOE ENVÍO PAGO A.B.5"
                error={errors.doe_envio_ab5}
                >
                <FInput
                    value={form.doe_envio_ab5}
                    onChange={e => set('doe_envio_ab5', e.target.value)}
                />
                </Field>
...
export default function EditarCompra3utmPage() {
  return <Suspense><EditarCompra3utmPageInner /></Suspense>;
}