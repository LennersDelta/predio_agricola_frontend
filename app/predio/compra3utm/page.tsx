// app/predio/compra3utm/page.tsx
'use client';
    ...
    <div onClick={e => { if (e.target === e.currentTarget) onCancel(); }}
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 14, padding: '32px 28px', maxWidth: 340, width: '90%', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,.1)' }}>
        ...
        <p style={{ fontSize: '.78rem', color: '#6b8f75', lineHeight: 1.6, marginBottom: 24 }}>
          Esta acción no se puede deshacer.<br />El compra 3 UTM será eliminado permanentemente.
        </p>
...
function Compra3UTMPageInner() {
  ...
  const [applied, setApplied] = useState({ predio: '',  estado: ''});
...
// ── Cargar datos ──────────────────────────────────────────────────────────
const cargaCompra3UTM = useCallback(() => {
  setLoading(true);

  api.get('/api/listaCompra3UTM')
    .then(({ data }) => {
      let datos = [];
    ...
    .catch((err) => {
      console.error(err);
      toast.error('Error al cargar compra 3 UTM efectuados');
      setData([]);
    })
    .finally(() => {
      setLoading(false);
...
useEffect(() => {
  ...
  const opPredios = [...new Set(data.map(b => b.predio_nombre).filter(Boolean))].sort();
    ...
    //  Aplicar filtros 
  const aplicar = () => {
    setApplied({predio: fPredio, estado: fEstado});
    setPage(1);
  };
    const limpiar = () => {setFPredio(''); setFEstado(''); 
    setApplied({predio: '', estado: '' });
      ...
      return data
        .filter(b => {
          return (
            ...
            (!applied.estado || Number(b.estado_id) === Number(applied.estado)) &&
    ...
    const handleDelete = async () => {
      ...
      try {
        await api.delete(`/api/deleteCompra3UTM/${deleteId}`);
        setData(prev => prev.filter(b => b.orden !== deleteId));
        toast.success('Compra 3 UTM eliminado correctamente', { id: toastId, duration: 3000 });
      } catch (err: any) {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24, padding: '0 4px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '3px 10px 3px 8px', borderRadius: 999, background: 'rgba(58,153,86,.1)', border: '1px solid rgba(58,153,86,.25)' }}>
            ...
            <span style={{ fontFamily: 'monospace', fontSize: '.58rem', fontWeight: 500, color: '#2e7d46', letterSpacing: '.12em', textTransform: 'uppercase' }}>Compras 3 UTM</span>
          </div>
          ...
          <p style={{ fontSize: '.72rem', color: '#3d5c47', fontFamily: 'monospace' }}>Gestión Compras 3 UTM</p>
        </div>
        {!loadingAuth && puedeCrear && (
          <Link href="/predio/compra3utm/crear"
        ...
        <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)', marginBottom: 16 }}>
          ...
          <div style={{ padding: '16px 20px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(145px,1fr))', gap: 12, alignItems: 'end' }}>
              ...
              {/* Compra desde hook */}
              <div>
                <label style={lblStyle}>Estado Compra</label>
                <select
                  value={fEstado}
                  onChange={e => { setFEstado(e.target.value); aplicar(); }}
                ...
                >
                  <option value="">{loadingEstadosCompra ? 'Cargando...' : 'Todos'}</option>
                  {EstadoCompra.map(p => (
          ...
          <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, padding: '13px 20px', borderBottom: '1px solid rgba(0,0,0,.06)', background: 'rgba(0,0,0,.02)' }}>
              <div>
                <p style={{ fontFamily: '"Barlow Condensed",sans-serif', fontSize: '.9rem', fontWeight: 700, color: '#1a2e22', textTransform: 'uppercase', letterSpacing: '.08em', lineHeight: 1 }}>Listado de Compras 3 UTM</p>
                        ...
                        <td style={{ padding: '10px 14px', verticalAlign: 'middle', textAlign: 'center' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                              <div style={{
                                ...
                                borderRadius: '50%',
                                background:
                                  b.estado_id === 6 ? '#16a34a' : '#dc2626'                                   
                              }} />
                              <span style={{ fontSize: '.78rem' }}>
                                <span style={{ fontFamily: 'monospace', fontSize: '.82rem', fontWeight: 700 }}>{b.estado_nombre}</span>
                              </span>
                            </div>
                        </td>
