...
export default function CrearFacturaAguaPage() {
  ...
  const [loading, setLoading] = useState(false);
    ...
    /* PARA LOS USUARIO CON UN PREDIO YA SELECCIONADO SE MODIFICA PARA QUE
    SE CUMPLA CON LA REGLA FROM.PREDIO */
    useEffect(() => {
      if (loadingPredios || predios.length === 0) return;
  
      // Usuario con un solo predio asignado
      if (predios.length === 1) {
        setForm(prev => ({
          ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const toastId = toast.loading('Guardando factura de agua...');

    try {
      await api.post('/api/factura/agua', {
        predio:               form.predio,
      ...
      toast.success('Factura de agua guardada correctamente.', { id: toastId, duration: 3000 });
      router.push('/predio/factura/agua');
