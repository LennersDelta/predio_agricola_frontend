// app/predio/factura/agua/page.tsx
'use client';
...
function FacturaAguaPageInner() {
  const [data,     setData]     = useState<FacturaAgua[]>([]);
  ...
  const cargar = useCallback(() => {
    setLoading(true);
    api.get('/api/factura/agua')
      .then(({ data: r }) => {
        const lista = Array.isArray(r) ? r : (r.data ?? []);
        setData([...lista]);
      })
      .catch(() => toast.error('Error al cargar facturas de agua'))
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/api/factura/agua/${deleteId}`);
    ...
    <div style={{ fontFamily: '"Barlow",sans-serif' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, padding: '13px 20px', borderBottom: '1px solid rgba(0,0,0,.06)', background: 'rgba(0,0,0,.02)' }}>
          <div>
            <p style={{ fontFamily: '"Barlow Condensed",sans-serif', fontSize: '.9rem', fontWeight: 700, color: '#1a2e22', textTransform: 'uppercase', letterSpacing: '.08em', lineHeight: 1 }}>Listado Facturas Agua</p>
...
export default function FacturaAguaPage() {
  return (
    <Suspense>
      <FacturaAguaPageInner />
    </Suspense>
  );
}