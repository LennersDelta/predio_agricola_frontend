...
// Tabla
  ...
  //  Cargar datos 
  const cargaFacturaAgua = useCallback(() => {
    setLoading(true);
    api.get('/api/predio')
      .then(({ data: r }) => setData(r.data ?? r))
      .catch(() => toast.error('Error al cargar factura de agua'))
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24, padding: '0 4px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '3px 10px 3px 8px', borderRadius: 999, background: 'rgba(58,153,86,.1)', border: '1px solid rgba(58,153,86,.25)' }}>
            ...
            <span style={{ fontFamily: 'monospace', fontSize: '.58rem', fontWeight: 500, color: '#2e7d46', letterSpacing: '.12em', textTransform: 'uppercase' }}>Factura Agua</span>
          </div>
          ...
          <p style={{ fontSize: '.72rem', color: '#3d5c47', fontFamily: 'monospace' }}>Gestión Factura Agua</p>
        </div>
