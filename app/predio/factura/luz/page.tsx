// app/predio/factura/luz/page.tsx
'use client';
    ...
    <div onClick={e => { if (e.target === e.currentTarget) onCancel(); }}
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 14, padding: '32px 28px', maxWidth: 340, width: '90%', textAlign: 'center', boxShadow: '0 4px 24px rgba(0,0,0,.1)' }}>
        ...
        <p style={{ fontSize: '.78rem', color: '#6b8f75', lineHeight: 1.6, marginBottom: 24 }}>
          Esta acción no se puede deshacer.<br />La factura será eliminada permanentemente.
        </p>
...
function FacturaLuzPageInner() {
  const [data,     setData]     = useState<FacturaLuz[]>([]);
  ...
  const cargar = useCallback(() => {
    setLoading(true);
    api.get('/api/factura/luz')
      .then(({ data: r }) => {
        const lista = Array.isArray(r) ? r : (r.data ?? []);
        setData([...lista]);
      })
      .catch(() => toast.error('Error al cargar facturas de luz'))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => { cargar(); }, [cargar]);

  // Opciones dinámicas mes consumo
  const opMesConsumo = [...new Set(data.map(b => b.mes_consumo).filter(Boolean))].sort();

  // Filtros
  const aplicar = () => { setApplied({ predio: fPredio, mesConsumo: fMesConsumo }); setPage(1); };
  const limpiar = () => { setFPredio(''); setFMesConsumo(''); setApplied({ predio: '', mesConsumo: '' }); setSearch(''); setPage(1); };
  ...
  const filtered = useMemo(() => {
    ...
    return data
      .filter(b => {
        const matchPredio     = !applied.predio     || b.predio?.toLowerCase().includes(applied.predio.toLowerCase());
        const matchMesConsumo = !applied.mesConsumo || b.mes_consumo?.includes(applied.mesConsumo);
        const matchSearch     = !q || [b.predio, b.n_factura, b.proveedor, b.doe, b.estado, b.consumo]
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/api/factura/luz/${deleteId}`);
      setData(prev => prev.filter(f => f.id !== deleteId));
      toast.success('Factura eliminada correctamente', { id: tid, duration: 3000 });
    } catch (err: any) {
      toast.error(err.response?.data?.message ?? 'Error al eliminar', { id: tid, duration: 5000 });
    } finally {
      setDeleteId(null);
    }
  };
  ...
  // Columnas de cabecera — [accessorKey, label, align]
  const cols: [string, string, string][] = [
    ['id',          'Orden',                  'left'],
    ['predio',      'Predio',              'left'],
    ['n_factura',   'N° Factura',          'left'],
    ['mes_consumo', 'Mes Consumo',         'center'],
    ['valor',       'Valor Total',         'right'],
    ['proveedor',   'Proveedor',           'left'],
    ['estado',      'Estado',              'center'],
    ['doe',         'DOE Resp. B.5',       'center'],
    ['consumo',     'Consumo (kg)',        'right'],
  ];
    ...
    <div style={{ fontFamily: '"Barlow",sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24, padding: '0 4px' }}>
        <div>
          ...
          <p style={{ fontSize: '.72rem', color: '#3d5c47', fontFamily: 'monospace' }}>Gestión de facturas de luz</p>
        </div>
        {!loadingAuth && puedeCrear && (
          <Link href="/predio/factura/luz/crear"
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)', marginBottom: 16 }}>
        ...
        <div style={{ padding: '16px 20px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(145px,1fr))', gap: 12, alignItems: 'end' }}>
            ...
            {/* Mes consumo dinámico */}
            <div>
              <label style={lblStyle}>Mes Consumo</label>
              <select
                value={fMesConsumo}
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, padding: '13px 20px', borderBottom: '1px solid rgba(0,0,0,.06)', background: 'rgba(0,0,0,.02)' }}>
          <div>
            <p style={{ fontFamily: '"Barlow Condensed",sans-serif', fontSize: '.9rem', fontWeight: 700, color: '#1a2e22', textTransform: 'uppercase', letterSpacing: '.08em', lineHeight: 1 }}>Listado Facturas Luz</p>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input type="text" placeholder="Buscar en tabla..." value={search}
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={cols.length + 1} style={{ padding: '40px', textAlign: 'center', color: '#9ab8a2', fontFamily: 'monospace', fontSize: '.78rem' }}>
                      No hay registros que coincidan.
                    </td>
                  </tr>
                ) : paginated.map(b => {
                  const pagada = b.estado?.toLowerCase().includes('paga');
                  return (
                    <tr key={b.id}
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '.72rem', color: '#9ab8a2' }}>#{b.id}</span>
                      </td>
                      {/* Predio */}
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '.78rem', fontWeight: 600, color: '#2e7d46' }}>{b.predio || '—'}</span>
                      </td>
                      {/* N° Factura */}
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '.82rem', fontWeight: 700, color: '#1a2e22' }}>{b.n_factura}</span>
                      </td>
                      {/* Mes Consumo */}
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle', textAlign: 'center' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '.78rem', color: '#3d5c47' }}>{fmtMes(b.mes_consumo)}</span>
                      </td>
                      {/* Valor */}
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle', textAlign: 'right' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '.82rem', fontWeight: 700, color: '#1a2e22' }}>{fmtPeso(b.valor)}</span>
                      </td>
                      {/* Proveedor */}
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle' }}>
                      ...
                      {/* Consumo */}
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle', textAlign: 'right' }}>
                        <span style={{ fontFamily: 'monospace', fontSize: '.82rem', color: '#1a2e22' }}>
                      ...
                      <td style={{ padding: '10px 14px', verticalAlign: 'middle', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                            ...
                            {!loadingAuth && puedeConsultar && (
                              <Link href={`/predio/factura/luz/${b.uuid}/ver`}
                            ...
                            {!loadingAuth && puedeEditar && ( 
                              <Link href={`/predio/factura/luz/${b.uuid}/edit`}
        ...
        {/* Paginación — key={label}-{i} para evitar duplicados */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, padding: '13px 20px', borderTop: '1px solid rgba(0,0,0,.04)' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[
              ...
              return (
                <button key={`${label}-${i}`} onClick={action}
                  style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 30, height: 30, padding: '0 8px', borderRadius: 6, fontFamily: 'monospace', fontSize: '.7rem', fontWeight: 600, border: `1px solid ${isActive ? 'rgba(58,153,86,.3)' : 'rgba(0,0,0,.1)'}`, background: isActive ? 'rgba(58,153,86,.08)' : '#fff', color: isActive ? '#2e7d46' : '#6b8f75', cursor: 'pointer' }}>
                  {label}
                </button>
              );
            })}
