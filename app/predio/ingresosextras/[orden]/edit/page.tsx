...
function EditarIngresosExtrasPageInner() {
    ...
    useEffect(() => {
        ...
        const cargar = async () => {
            try {
                ...
                const { data } = await api.get(`/api/ingresosextras/${uuid}`);
                ...
                setForm({
                    ...
                    item_venta: data.data.item_venta ?? '',
                    dte_resolucion: data.data.dte_resolucion ?? '',
                    valor_total: String(data.data.valor_total ?? ''),
                    fecha: data.data.fecha ?? '',

                    estado_pago: String(data.data.estado_pago ?? ''),
    ...
    const validate = () => {
        ...
        if (!form.item_venta)
            newErrors.item_venta = 'Debe ingresar el ítem de venta.';

        if (!form.dte_resolucion)
            newErrors.dte_resolucion = 'Debe ingresar el DTE o Resolución.';

        if (!form.valor_total)
            newErrors.valor_total = 'Debe ingresar el valor total.';

        if (!form.fecha)
            newErrors.fecha = 'Debe ingresar la fecha.';

        if (!form.estado_pago)
            newErrors.estado_pago = 'Debe seleccionar el estado de pago.';

        if (!form.doe_informa_ab5)
            newErrors.doe_informa_ab5 = 'Debe ingresar el DOE informó AB5.';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            formData.append('item_venta', form.item_venta);
            formData.append('dte_resolucion', form.dte_resolucion);
            formData.append('valor_total', form.valor_total);
            ...
            await api.post(`/api/ingresosextras/${uuid}`, formData, {
                headers: {
...
export default function EditarIngresosExtrasPage() {
  return <Suspense><EditarIngresosExtrasPageInner /></Suspense>;
}