// app/predio/insumosproducto/page.tsx
'use client';
...
const opTiposCompra = [...new Set(data.map(b => b.tipo_compra).filter(Boolean))].sort();
const opEstadosOC = [...new Set(data.map(b => b.estado_orden).filter(Boolean))].sort();
const opEstadosFactura = [...new Set(data.map(b => b.estado_factura).filter(Boolean))].sort();

  // ── Aplicar filtros ───────────────────────────────────────────────────────
  const aplicar = () => {
    setApplied({orden: fOrden, predio: fPredio, tipoCompra: fTipoCompra, estadoOrdenCompra: fEstadoOC,  estadoFactura: fEstadoFactura });
    setPage(1);
  };
    const limpiar = () => { setFOrden(''); setFPredio(''); setFTipoCompra(''); setFEstadoOC(''); setFEstadoFactura('');
    setApplied({orden: '', predio: '', tipoCompra: '',  estadoOrdenCompra: '',  estadoFactura: ''  });
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24, padding: '0 4px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 8, padding: '3px 10px 3px 8px', borderRadius: 999, background: 'rgba(58,153,86,.1)', border: '1px solid rgba(58,153,86,.25)' }}>
            ...
            <span style={{ fontFamily: 'monospace', fontSize: '.58rem', fontWeight: 500, color: '#2e7d46', letterSpacing: '.12em', textTransform: 'uppercase' }}>Aquisición de insumos y productos</span>
          </div>
    ...
    <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)', marginBottom: 16 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,.1)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,.08)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, padding: '13px 20px', borderBottom: '1px solid rgba(0,0,0,.06)', background: 'rgba(0,0,0,.02)' }}>
          <div>
            <p style={{ fontFamily: '"Barlow Condensed",sans-serif', fontSize: '.9rem', fontWeight: 700, color: '#1a2e22', textTransform: 'uppercase', letterSpacing: '.08em', lineHeight: 1 }}>Listado de insumos y productos</p>
              ...
              <thead>
                <tr>
                  {([
                    ...
                    ['valor_total', 'Valor Total Pagado ' , 'center'],
...
export default function InsumosProductoPage() {
  return (
    <Suspense>
      <InsumosProductoPageInner />
    </Suspense>
  );
}