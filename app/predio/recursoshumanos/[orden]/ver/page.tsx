...
const set = (k: string, v: string) => {
    ...
    const cargar = async () => {
      try {
        ...
        setForm({
                ...
                capacitado_prevencion_riesgo: b.capacitado_prevencion_riesgo ? '1' : '0'
        });
  ...
  const validate = () => {
    ...
    if (!form.predio_id) newErrors.predio_id = 'El predio es obligatorio';
    if (!form.grado_id) newErrors.grado_id = 'El grado es obligatorio';
    if (!form.tipo_contrato_id) newErrors.tipo_contrato_id = 'El tipo de contrato es obligatorio';
    if (!form.nombres_apellidos.trim()) newErrors.nombres_apellidos = 'Nombre obligatorio';
    if (!form.rut.trim()) newErrors.rut = 'RUT obligatorio';
    if (!form.cargo_contratado.trim()) newErrors.cargo_contratado = 'Cargo obligatorio';
    if (!form.area_funciones.trim()) newErrors.area_funciones = 'Área obligatoria';
    if (!form.funcion_actual.trim()) newErrors.funcion_actual = 'Función obligatoria';
    if (!form.anios_servicio.trim()) newErrors.anios_servicio = 'Años servicio obligatorio';
    if(!form.ultima_calificacion.trim()) newErrors.ultima_calificacion = 'Debe agregar ultima calificación';
    if (!form.capacitado_prevencion_riesgo) newErrors.capacitado_prevencion_riesgo = 'Seleccione una opción';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formData.append('grado_id', String(form.grado_id));
      formData.append('nombres_apellidos', form.nombres_apellidos);
      formData.append('rut', form.rut);

      formData.append('cargo_contratado', form.cargo_contratado);
      formData.append('area_funciones', form.area_funciones);
      formData.append('funcion_actual', form.funcion_actual);

      if (form.fecha_inicio_contrato) {
        formData.append('fecha_inicio_contrato', form.fecha_inicio_contrato);
      }
      if (form.anios_servicio !== undefined && form.anios_servicio !== '') {
        formData.append('anios_servicio', String(form.anios_servicio));
      }
      if (form.ultima_calificacion) {
        formData.append('ultima_calificacion', form.ultima_calificacion);
      }
      formData.append(
        'capacitado_prevencion_riesgo',
        String(Number(form.capacitado_prevencion_riesgo)) 
      );

      formData.append('tipo_contrato_id', String(form.tipo_contrato_id));

      await api.post(
        `/api/recursoshumanos/${uuid}`,
        formData,
        {
          headers: {
              ...
              {/* SECCIÓN 2 — PERSONAL */}
                <Section>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 16 }}>
                    ...
                    <Field label="RUT" error={errors.rut}>
                        <FInput
                          ...
                          onChange={e => {
                            ...
                            if (value.length > 7 && !validarRut(value)) {
                    ...
                    <Field label="Tipo Contrato" error={errors.tipo_contrato_id}>
                      <FSelect
                    ...
                    <Field label="Años de Servicio" error={errors.anios_servicio}>
                      <FInput
                        type="number"
