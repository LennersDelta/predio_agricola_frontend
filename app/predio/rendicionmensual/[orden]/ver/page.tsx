...
function VerRendicionMensualPageInner() {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      try {
        ...
        console.error(error);
        toast.error('No se pudo cargar la rendición mensual');
      } finally {
        setCargando(false);
      }
    };cargar();
...
export default function VerRendicionMensualPage() {
  return (
    <Suspense>
      <VerRendicionMensualPageInner />
    </Suspense>
  );
}